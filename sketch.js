...
// Sensors state (false until user enables on mobile)
window.sensorsEnabled = false;

// Device orientation handler: map native events to the globals this sketch expects
function handleDeviceOrientation(evt) {
  // evt.beta -> rotation around X axis (front-to-back tilt)
  // evt.gamma -> rotation around Y axis (left-to-right tilt)
  // evt.alpha -> rotation around Z axis (compass/rotation)
  window.rotationX = (typeof evt.beta === 'number') ? evt.beta : 0;
  window.rotationY = (typeof evt.gamma === 'number') ? evt.gamma : 0;
  window.rotationZ = (typeof evt.alpha === 'number') ? evt.alpha : 0;
}

// Show a tap overlay and request permission when tapped (iOS) or just enable on other platforms
function enableGyroTap(message) {
  // If sensors are already enabled, do nothing
  if (window.sensorsEnabled) return;

  const overlay = document.createElement('div');
  overlay.id = 'gyro-enable-overlay';
  ...
  overlay.innerHTML = `<div style="max-width:80%;">${message || 'Tap to enable device rotation'}</div>`;
  ...
  async function onClick(e) {
    // Try iOS 13+ style permission first (DeviceOrientationEvent)
    try {
      if (typeof DeviceOrientationEvent !== 'undefined' &&
        ...
        if (resp === 'granted') {
          ...
          // permission denied
          console.warn('DeviceOrientation permission denied:', resp);
        }
      } else if (typeof DeviceMotionEvent !== 'undefined' &&
                 typeof DeviceMotionEvent.requestPermission === 'function') {
        // Some devices use DeviceMotionEvent for motion permission
        ...
        // Non-iOS / desktop: no permission needed
        window.addEventListener('deviceorientation', handleDeviceOrientation);
        window.sensorsEnabled = true;
        removeOverlay();
        return;
      ...
      console.warn('Error while requesting device orientation permission', err);
    }

    // If we reach here, permission was not granted — show a notice then remove overlay after a moment
    overlay.innerHTML = '<div style="max-width:80%;">Permission was not granted. Tap to try again.</div>';
    // leave overlay so user can tap again
  }
...
//function preload() {
 //   gif1 = loadImage('Fortnite_Trip_Out.gif');
 //   gif2 = loadImage('peely_fortnite.gif');
//}
...
function setup() {
  ...
  if (typeof lockGestures !== 'function') {
    // Stub for lockGestures if not defined elsewhere
    window.lockGestures = function() {
      addLog('lockGestures() stub called (no-op)');
    };
  }
  lockGestures();
  
  addLog('Calling enableGyroTap()...');
  if (typeof enableGyroTap !== 'function') {
    // Stub for enableGyroTap if not defined elsewhere
    window.enableGyroTap = function(message) {
      addLog('enableGyroTap() stub called (no-op): ' + message);
    };
  }
  if (typeof enableGyroTap === 'function') {
